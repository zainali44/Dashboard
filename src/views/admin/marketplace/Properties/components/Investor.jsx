...
export default function Investor() {
    ...
    const handleShare = (index) => {
        // Update the status to "Shared" for the clicked investor
        const updatedInvestors = [...investors];
        updatedInvestors[index].status = "Shared";
                                    ...
                                    <tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                        {investors.map((investor, index) => (
                                            <tr key={investor.email} className="text-gray-700 dark:text-gray-400">
                                                ...
                                                <td className="px-4 py-3 text-sm">{investor.investment}</td>
                                                <td className="px-4 py-3 text-xs">
                                                    <span
                                                        className={`px-2 py-1 font-semibold leading-tight rounded-full ${investor.status === "Pending"
                                                            ? "text-yellow-700 bg-yellow-100"
                                                            : investor.status === "Shared"
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center space-x-4 text-sm">
                                                        <button
                                                            ...
                                                            className={`flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-green-700 ${investor.status === "Shared" ? "cursor-not-allowed" : ""}`}
